...
export default function MelanomaCaseStudyView({ plan, output, onPlanChange, onOutput, onComplete, onControlled, onToast }: { plan: MelanomaWorkflowPlan; output: MelanomaWorkflowOutput | null; onPlanChange: (plan: MelanomaWorkflowPlan) => void; onOutput: (output: MelanomaWorkflowOutput | null) => void; onComplete: (result: ReportExecutionResult) => void; onControlled: () => void; onToast: (message: string) => void }) {
  ...
  const scrollTo = (id: "example-plan" | "example-run") => window.setTimeout(() => document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
  return <div className="view melanoma-lab-view example-page">
    <header className="case-header"><div className="case-id"><span>INTERACTIVE CANCER EXAMPLE</span><strong>EX-MEL-01</strong><i>SYNTHETIC · SEED {MELANOMA_DEMO_SEED}</i></div><span className={`case-decision ${output ? "complete" : "pending"}`}>{output ? "ANALYSIS COMPLETE" : "YOU CONTROL THE RUN"}</span></header>
    <section className="example-hero"><div><span className="page-kicker">Example · melanoma tumor microenvironment</span><h1>Choose the methods and run the complete example here.</h1><p>The dataset, analysis choices, AI guidance, execution, interpretation, and downloads now live on this single page. Nothing runs until you select modules, confirm the plan, and press Run.</p><div><button className="primary-button" onClick={() => scrollTo("example-plan")}>Choose analyses and methods <span>↓</span></button><small>JavaScript and genuine browser-R methods are available</small></div></div><aside><span>RESEARCHER ASKS</span><blockquote>Which baseline melanoma expression features and tumor-microenvironment programs are associated with synthetic PD-1 response after accounting for clinical and technical covariates?</blockquote><dl><div><dt>Population</dt><dd>180 baseline tumors</dd></div><div><dt>Outcome</dt><dd>Synthetic response</dd></div><div><dt>Data</dt><dd>Bulk RNA-seq counts</dd></div><div><dt>Covariates</dt><dd>7 declared</dd></div></dl></aside></section>
    ...
    <section className="example-choice-preview"><div><span>YOUR NEXT STEP</span><h2>Select exactly what this example should run.</h2><p>Choose browser DGE, genuine R-package DGE, program summaries, tumor-purity sensitivity, neural integration, or any combination. The method guide reacts to every choice.</p></div><button className="primary-button" onClick={() => scrollTo("example-plan")}>Start selecting <span>↓</span></button></section>
    <section id="example-plan" className="example-workflow-anchor"><MelanomaAnalysisPlanView plan={plan} onPlanChange={onPlanChange} onRun={() => scrollTo("example-run")} onControlled={onControlled} onToast={onToast} /></section>
    {plan.confirmed && <section id="example-run" className="example-workflow-anchor"><MelanomaExecutionView plan={plan} output={output} onOutput={onOutput} onComplete={onComplete} onEditPlan={() => scrollTo("example-plan")} onControlled={onControlled} onToast={onToast} /></section>}
  </div>;
}
...
export function MelanomaAnalysisPlanView({ plan, onPlanChange, onRun, onControlled, onToast }: { plan: MelanomaWorkflowPlan; onPlanChange: (plan: MelanomaWorkflowPlan) => void; onRun: () => void; onControlled: () => void; onToast: (message: string) => void }) {
  ...
  const toggleRMethod = (id: WebRMethodId) => update({ r_methods: plan.r_methods.includes(id) ? plan.r_methods.filter((item) => item !== id) : [...plan.r_methods, id] });
  const confirm = () => {
  ...
  return <div className="view melanoma-lab-view analysis-planner-view">
    <div className="page-head"><div><span className="page-kicker">Step 1 · select the example analysis</span><h1>Choose exactly what will run.</h1><p>Every module and method below is a working control. Selected cards have a green border and checkmark; click them again to remove them.</p></div><span className={`planner-status ${plan.confirmed ? "confirmed" : "draft"}`}>{plan.confirmed ? "PLAN CONFIRMED" : "SELECT YOUR METHODS"}</span></div>
...
export function MelanomaExecutionView({ plan, output, onOutput, onComplete, onEditPlan, onControlled, onToast }: { plan: MelanomaWorkflowPlan; output: MelanomaWorkflowOutput | null; onOutput: (output: MelanomaWorkflowOutput | null) => void; onComplete: (result: ReportExecutionResult) => void; onEditPlan: () => void; onControlled: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="view melanoma-lab-view execution-workflow-view">
    <header className="execution-plan-header"><div><span>STEP 2 · CONFIRMED EXAMPLE PLAN</span><h1>Run the analysis you selected.</h1><p>{plan.research_question}</p></div><button onClick={onEditPlan}>Change selections</button></header>

...
export default function MelanomaCaseStudyView({ plan, output, onPlanChange, onOutput, onComplete, onControlled, onToast }: { plan: MelanomaWorkflowPlan; output: MelanomaWorkflowOutput | null; onPlanChange: (plan: MelanomaWorkflowPlan) => void; onOutput: (output: MelanomaWorkflowOutput | null) => void; onComplete: (result: ReportExecutionResult) => void; onControlled: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="view melanoma-lab-view example-page">
    <header className="case-header"><div className="case-id"><span>ONE WORKED EXAMPLE</span><strong>EX-MEL-01</strong><i>SYNTHETIC · SEED {MELANOMA_DEMO_SEED}</i></div><span className={`case-decision ${output ? "complete" : "pending"}`}>{output ? "ANALYSIS COMPLETE" : "YOU CONTROL THE RUN"}</span></header>
    <section className="example-hero"><div><span className="page-kicker">Example</span><h1>Run one melanoma RNA-seq question from data profile to report.</h1><p>This is the only built-in example. Inspect the synthetic dataset, choose the analysis modules and methods, confirm the plan, run it, compare the outputs, and download the record on this page.</p><div><button className="primary-button" onClick={() => scrollTo("example-plan")}>Choose analyses and methods <span>↓</span></button><small>JavaScript and genuine browser-R methods are available</small></div></div><aside><span>SYNTHETIC MELANOMA STUDY</span><blockquote>Which baseline expression features and tumor-microenvironment programs are associated with synthetic PD-1 response after accounting for clinical and technical covariates?</blockquote><dl><div><dt>Population</dt><dd>180 baseline tumors</dd></div><div><dt>Outcome</dt><dd>Synthetic response</dd></div><div><dt>Data</dt><dd>Bulk RNA-seq counts</dd></div><div><dt>Covariates</dt><dd>7 declared</dd></div></dl></aside></section>
...
export function MelanomaAnalysisPlanView({ plan, onPlanChange, onRun, onControlled, onToast }: { plan: MelanomaWorkflowPlan; onPlanChange: (plan: MelanomaWorkflowPlan) => void; onRun: () => void; onControlled: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="view melanoma-lab-view analysis-planner-view">
    <div className="page-head"><div><span className="page-kicker">Example · build the plan</span><h1>Choose exactly what will run.</h1><p>Every module and method below is a working control. Selected cards have a green border and checkmark; click them again to remove them.</p></div><span className={`planner-status ${plan.confirmed ? "confirmed" : "draft"}`}>{plan.confirmed ? "PLAN CONFIRMED" : "SELECT YOUR METHODS"}</span></div>
